...
Evaluated on a sealed, hash-verified final holdout benchmark ($n = 867$, comprising 249 attacks, 543 benign, and 75 adversarial evasions after deduplication), RAG-Shield achieves a Prevention Attack Detection Rate (ADR) of \textbf{87.04\%} and a Detection ADR of \textbf{87.35\%} at a False Positive Rate of only \textbf{0.37\%} (2/543 benign queries). The system achieves F1 scores of \textbf{92.76\%} under the prevention policy and \textbf{92.94\%} under the detection policy, with ROC-AUC of \textbf{0.9643} and AUC-PR of \textbf{0.9701}. In head-to-head comparisons against commercial alternatives on a separate shared subset, RAG-Shield (ADR = 93.54\%, F1 = 0.945) outperforms Llama Prompt Guard 2 (ADR = 79.49\%, F1 = 0.883, $p < 10^{-4}$), Llama-3.1-8B Guardrails (ADR = 29.78\%), and NVIDIA NeMo Injection Rails (ADR = 7.58\%). Final-holdout mean local pipeline latency is \textbf{1,214 ms}; live commercial-comparison latency for RAG-Shield is \textbf{2,187 ms} including the comparison protocol overhead.
\end{abstract}
...
\begin{enumerate}
\item \textbf{Multi-Signal Meta-Aggregation:} Instead of hard-threshold veto gates, we train a calibrated Logistic Regression meta-aggregator on a 10-dimensional evidence vector derived from all layers, enabling principled uncertainty handling and tunable operating points without retraining any component model.
\end{enumerate}
...
\caption{RAG-Shield architecture. Layers 1 and 2 execute in parallel. An early-exit gate on Layer 2 provides immediate blocking at high confidence. The meta-aggregator combines a 10-dimensional evidence vector into a calibrated risk probability.}
\label{fig:1}
\end{figure}
...
\textbf{Early Exit.} When \texttt{ENABLE\_L2\_EARLY\_EXIT = True} and $S_{L2} \geq \tau_{\text{early\_exit}}$, the orchestrator immediately blocks the request before invoking downstream checks. In the final holdout evaluation, \textbf{284 total samples} triggered the early-exit path; \textbf{282} of these were true-positive prevention decisions and \textbf{2} were benign false positives.


\textbf{Layer Attribution.} Layer 2 provided \textbf{260 of the 282 true-positive prevention attributions} in the final holdout, confirming it as the pipeline's primary detection engine.
...
The meta-aggregator is a \texttt{CalibratedClassifierCV}-wrapped \texttt{LogisticRegressionCV} model (scikit-learn) fitted on pipeline activation logs from development runs. It accepts a \textbf{10-dimensional feature vector} $\mathbf{f}$ constructed from layer outputs:
...
The final holdout benchmark was constructed under a strict \textbf{no-contamination protocol}: all samples were hash-deduplicated against training and development sets. File integrity is verified at evaluation time via SHA-256 checksums recorded in \texttt{logs/frozen\_thresholds.json}. The raw files contain 868 rows; the evaluator removes one duplicate attack row, yielding 867 evaluated samples. The benchmark is split across three files:
...
\midrule
\texttt{attacks.jsonl} & 249 & Blind post-freeze holdout generation & Direct + indirect injection payloads \\
...
\begin{table*}[!t]
\centering
\caption{RAG-Shield Final Holdout Benchmark ($n = 867$, frozen thresholds, Git \texttt{3ef1522})}
\label{tab:5}
\scriptsize
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{lccc}
...
\textbf{ADR} & \textbf{87.04\%} & \textbf{87.35\%} & Prev. [82.94\%, 90.26\%]; Det. [83.28\%, 90.53\%] \\
\textbf{FPR} & \textbf{0.37\%} & \textbf{0.37\%} & [0.10\%, 1.33\%] \\
\textbf{Precision} & \textbf{99.30\%} & \textbf{99.30\%} & Prev. [97.47\%, 99.81\%]; Det. [97.48\%, 99.81\%] \\
\textbf{F1 Score} & \textbf{92.76\%} & \textbf{92.94\%} & Prev. [90.59\%, 94.84\%]; Det. [90.66\%, 94.86\%] \\
\textbf{ROC-AUC} & 0.9643 & 0.9643 & [0.9484, 0.9777] \\
\textbf{AUC-PR} & 0.9701 & 0.9701 & -- \\
...
\caption{Final-holdout ROC and precision-recall curves generated from \texttt{logs/curves/roc\_final\_holdout.png} and \texttt{logs/curves/pr\_final\_holdout.png}.}
\label{fig:2}
\end{figure*}
...
\begin{table*}[!t]
\centering
\caption{Per-Layer Latency Breakdown (Final Holdout, $n = 867$)}
\label{tab:6}
\scriptsize
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{lcc}
...
\textbf{Total Pipeline} & \textbf{1,211.93} & \textbf{4,633.85} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table*}
...
Cache efficiency was high: L1 registered \textbf{692 cache hits} and L2 document scanning registered \textbf{692 cache hits} across the 867 evaluation runs, corresponding to a cache hit rate of approximately 79.8\%.


\subsection{Layer Attribution}

Layer 2 (Intent Classifier) was the dominant blocking layer, receiving \textbf{260 of 282 true-positive prevention attributions} (92.2\%) in the final holdout. Layer 1 (Anomaly Detector) received \textbf{22 true-positive attributions} (7.8\%). The ablation study below shows that the marginal prevention gain over Layer 2 alone is smaller: L1+L2 catches one additional attack relative to L2-only under the frozen policy.
...
Prompt injection represents a fundamental security vulnerability in RAG architectures that cannot be addressed by any single checkpoint or technique. We have presented \textbf{RAG-Shield}, a three-layer ensemble defense that intercepts the RAG pipeline at the retrieval input, generation context, and model output simultaneously. Through the combination of embedding-based OOD anomaly detection, transformer-based dual-stage intent classification with document-level chunk scanning, and output behavioral monitoring with cryptographic canary honeypots--orchestrated by a calibrated 10-dimensional meta-aggregator--RAG-Shield achieves a comprehensive attack detection rate of \textbf{87.35\%} at an enterprise-grade False Positive Rate of \textbf{0.37\%}.


Our ablation study shows that the full prevention policy improves by \textbf{0.31 pp} over Layer 2 alone (87.04\% vs. 86.73\%), while the full detection policy adds one monitored true positive (87.35\%). This means the current final-holdout result is driven primarily by document-level DeBERTa scanning, with Layer 1 adding a small number of structurally anomalous detections and Layer 3 requiring separate canary/output and multi-turn benchmarks for full evaluation. Head-to-head comparisons against commercial guardrails show a \textbf{+14.05 pp ADR advantage over Llama Prompt Guard 2} and a \textbf{+63.76 pp advantage over Llama-3.1-8B Guardrails} on the live comparison subset.
